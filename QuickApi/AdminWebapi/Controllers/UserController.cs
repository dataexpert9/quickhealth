...
namespace AdminWebapi.Controllers
{
    ...
    public class UserController : ApiController
    {
        ...
        [Route("MarkVerified")]
        [HttpPost]
        public async Task<IHttpActionResult> MarkUserAccountAsVerified(UserModel model)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);

            User userModel = _UserService.MarkUserAccountAsVerified(model);
        ...
        public async Task<IHttpActionResult> AddFamilyMember(EditUserProfileBindingModel model)
        {
            try
            {
                if (!ModelState.IsValid)
        ...
        public async Task<IHttpActionResult> UpdateNotificationStatus(bool Status, string Email)
        {
            try
            {
                ...
                if (user)
                {
                    return Ok(new CustomResponse<string> { Message = GlobalUtility.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = "Profile updated successfully." });
                }
                else
                {
        ...
        public async Task<IHttpActionResult> GetFamilyMembers()
        {
            try
            {
                ...
                foreach (var item in returnModel.FamilyMembers)
                {
                    item.CalculateFamilyMemberProfilePercentage();
                }
        ...
        public async Task<IHttpActionResult> GetMyCases(int? User_Id = 0)
        {
            try
            {
                ...
                GetMyCases response = new GetMyCases();
                ...
                var Inquiries = _UserService.GetMyCases(User_Id.Value);
                List<AppointmentHistoryViewModel> returnModel = Mapper.Map<List<Appointment>, List<AppointmentHistoryViewModel>>(Inquiries);
                response.Appointments = returnModel;
                return Ok(new CustomResponse<GetMyCases> { Message = GlobalUtility.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = response });
        ...
        //[Route("Dispatch")]
        //[HttpPost]
        //public async Task<IHttpActionResult> Dispatch(AppointmentDispatchBindingModel model)
        //{
        //    try
        //    {
        //        AppointmentViewModel returnModel = new AppointmentViewModel();
        ...
        //            return BadRequest(ModelState);
        //        }
        //        returnModel.Appointment = _UserService.Dispatch(model);
        //        if (returnModel.Appointment != null)
        //        {
        //            return Ok(new CustomResponse<AppointmentViewModel> { Message = GlobalUtility.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = returnModel });
        //        }
