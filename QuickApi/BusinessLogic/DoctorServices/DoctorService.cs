...
{
    public class AdminService : IDoctorService
    {
        ...
        private readonly GenericRepository<DoctorRating> _DoctorRatingRepository;
        ...
        public AdminService()
        {
            ...
            _DoctorRatingRepository = new GenericRepository<DoctorRating>(_DBContext);
        ...
        public Doctor UpdateDoctorProfile(EditDoctorProfileBindingModel model)
        {
            ...
            if (doctorModel != null)
            {
                ...
                doctorModel.DateofBirth = model.DateOfBirth;
                doctorModel.Phone = model.PhoneNumber;
        ...
        public List<Appointment> GetRequestQueries()
        {
            ...
            var AppointmentList = _AppointmentRepository.GetWithIncludeList(x => x.Status == (int)Utility.AppointmentStatus.Pending, "User", "FamilyMember", "User.Medications", "AppointmentImages", "AllergyHistory", "VaccinationHistory", "MedicationHistory", "MedicalConditionHistory", "LifeStyleHistory").OrderByDescending(x=>x.Id).ToList();
            return AppointmentList;
        }

        public List<Appointment> GetHistory(int Doctor_Id)
        {
            var DoctorHistory = _AppointmentRepository.GetWithInclude(x=>x.Doctor_Id==Doctor_Id && x.Status==(int)Utility.AppointmentStatus.Completed,"User", "FamilyMember", "AppointmentImages", "DoctorPrescription", "DoctorPrescription.DoctorPrescriptionImages", "AllergyHistory", "VaccinationHistory", "MedicationHistory", "MedicalConditionHistory", "LifeStyleHistory").OrderByDescending(x=>x.Id).ToList();
        ...
        public Appointment AcceptAppointment(AcceptAppointmentBindingModel model)
        {
            ...
            if (Appointment.Doctor_Id == null)
            {
                ...
                Appointment.Status = (int)AppointmentStatus.Assigned;
                _AppointmentRepository.Save();

                Appointment = _AppointmentRepository.GetFirstWithInclude(x=>x.Id==Appointment.Id, "User", "FamilyMember", "AppointmentImages", "DoctorPrescription", "DoctorPrescription.DoctorPrescriptionImages", "AllergyHistory", "VaccinationHistory", "MedicationHistory", "MedicalConditionHistory", "LifeStyleHistory");
                return Appointment;
            }
            else
                return null;
        ...
        public Boolean DoctorPrescriptions(DoctorPrescriptionBindingModfel model)
        {
            
            var Appointment = _AppointmentRepository.GetFirst(x=>x.Id==model.Appointment_Id && x.Doctor_Id==model.Doctor_Id);
            if (Appointment == null)
                return false;
            ...
            prescription.Appointment_Id = model.Appointment_Id;
            prescription.PrescriptionNote = model.PrescriptionNote;

            _DoctorPrescriptionRepository.Insert(prescription);
            _DoctorPrescriptionRepository.Save();


            if (prescription.DoctorPrescriptionImages == null)
                prescription.DoctorPrescriptionImages = new List<DoctorPrescriptionImages>();

            foreach (var file in model.PrescriptionImages)
            {

                if (file != null)
                {
                    
                    var ImageUrl = ImageHelper.SaveFileFromBytes(file, "DoctorPrescription");

                    prescription.DoctorPrescriptionImages.Add(new DoctorPrescriptionImages
                    {
                        DoctorPrescription_Id= prescription.Id,
                        ImageUrl = ImageUrl
                    });
                }
            }
        ...
        public Doctor GetDoctorProfile(int Doctor_Id)
        {

            Doctor returnModelDoctor = new Doctor();
            returnModelDoctor = _DoctorRepository.GetFirst(x => x.Id == Doctor_Id && x.IsDeleted == false);
            return returnModelDoctor;
        }
        
        public bool DoctorCloseChat(DoctorCloseChatViewModel model)
        {
            var Appointment = _AppointmentRepository.GetFirst(x=>x.Id==model.Appointment_Id && x.Doctor_Id==model.Doctor_Id);
            if (Appointment == null)
                return false;
            ...
            { 
                ...
                Appointment.Status = (int)Utility.AppointmentStatus.Completed;
                _AppointmentRepository.Save();
            }
        ...
        public bool RateDoctor(RateDoctorBindingModel model)
        {

            DoctorRating rating = new DoctorRating();
            rating.Doctor_Id = model.Doctor_Id;
            rating.Rating = model.Rating;
            rating.Feedback = model.Feedback;
            rating.User_Id = model.User_Id;
            _DoctorRatingRepository.Insert(rating);
